...
async def _nim_chat(
    ...
    logger.info("Making NIM API call with model %s", model)
    pprint(messages)
    payload = {
        "model": model,
        "messages": messages,
...
async def _estimate_price(title: str, desc: str) -> float:
    ...
    try:
        price_str = await _nim_chat(settings.LLM_MODEL, messages)
...
async def negotiate(
    ...
    # Get AI response
    try:
        response = await _nim_chat(settings.LLM_MODEL, messages)
